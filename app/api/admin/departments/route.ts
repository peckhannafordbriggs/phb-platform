...
import { createDepartment, listDepartments } from "@/lib/admin/service";
...
export async function GET(request: NextRequest) {
  return withAdmin("/api/admin/departments", async () => {
    const includeHidden =
      request.nextUrl.searchParams.get("includeHidden") === "true";
    return ok(await listDepartments(includeHidden));
  });
}

export async function POST(request: Request) {
  return withAdmin("/api/admin/departments", async (viewer) => {
    ...
    return ok(await createDepartment(viewer.id, parsed.data.name), 201);
  });
}
